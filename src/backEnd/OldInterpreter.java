...
public class OldInterpreter {
	 ...
	 */
	public OldInterpreter(ModelModifiable m) {
		...
		ArrayList<String> myTempArgs = new ArrayList<>();
			...
			while(myTempArgs.size() < myExpectedSyntax) {
				...
				if(!myInputArgs.isEmpty() && myLanguageProperties.containsKey(myInputArgs.get(0))) {
					...
					String myCommand = myShortCommands.getProperty(myLanguageProperties.getProperty(myInternalTempArgs.get(0)));
					String[] syntax = getCommandSyntax(myCommand);
					int syntaxLength = syntax.length;
					while(syntaxLength > myInternalTempArgs.size()) {
						if(myLanguageProperties.containsKey(myInputArgs.get(0))) {
							syntaxLength += getCommandSyntax(myShortCommands.getProperty(myLanguageProperties.getProperty(myInputArgs.get(0)))).length;
						}
						myInternalTempArgs.add(myInputArgs.remove(0));
